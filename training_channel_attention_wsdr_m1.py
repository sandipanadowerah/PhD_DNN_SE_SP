...
from data_utils_ds_m1 import get_UnetDataset, collate_fn_speech_enhancement
from unet import U_Net, U_Net_Attention, U_Net_Channel_Attention
from loss_functions_wsdr import Unet2Loss
    ...
    for epoch in range(start_epoch+1, num_epochs):
        for phase in ['train']:
            ...
            for i, data in enumerate(dataloader[phase]):
                y_stft, s_stft, mask, s_dry, y = data
                s_stft = s_stft.unsqueeze(0)
                y_stft = y_stft.unsqueeze(0).permute(0, 3, 2, 1)
                mask = mask.unsqueeze(0).permute(0, 3, 2, 1)

                #print(y_stft.shape, mask.shape)
                
                _, r = divmod(y_stft.shape[2], 8)
                r = 8 - r
                add = torch.zeros(1,4,r,257).double()
                input_ = torch.cat((y_stft, add), dim=2)

                #print(input_.shape)

                if use_cuda:
                    input_, s_stft, mask, y_stft = input_.to(device), s_stft.to(device), mask.to(device), y_stft.to(device)
                    s_dry, y = s_dry.to(device), y.to(device)

                pred_mask = model(input_)
                pred_mask = pred_mask[:,:,:mask.shape[2],:]
                #y_stft, s_stft, mask, pred_mask
                loss = criterion(y_stft, s_stft, mask, pred_mask, s_dry, y)
                optimizer.zero_grad()
